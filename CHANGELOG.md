...
## v3.7（2026-08-26）— 子技能运行期反馈回传 + 工厂自动归因（元进化闭环补全）

- **动机**：子技能在真实使用中遇到"自己解决不了的问题"时，之前只有"内容类反馈 → 自身升级环"半截链路，机制/宿主类反馈**无通道回传工厂**——工厂不知道，用户只能手动转述。本次补齐「子技能 → 工厂」反馈闭环。
- **三类问题分诊（回传前先归因）**：
  - `content` 内容类（改子技能自身规则/知识/示例能修）→ 子技能走升级环（evolution-protocol §3），不回传工厂；
  - `mechanism` 机制类（只有改**工厂产出逻辑**才能根治：SKILL.md 结构缺字段 / schema 不兼容 / 脚本报错 / few-shot 指引导致调用方行为错误）→ **回传工厂** → 进 `factory-log.md` 待办；
  - `host` 宿主类（换环境/重启/授权/换通道能解，如 C 类沙箱通道故障）→ 生成宿主 Bug Report 模板交用户上报，**不当作工厂缺陷**（与 §17.1 C 类同判）。
- **双通道回传**：
  - 推模式：子技能升级环执行时自动写 `references/memory/feedback/factory-feedback.json`（含 symptom / cause_type / self_resolved / detail / gap_ref）；
  - 拉模式：工厂每次被调用时**先征得用户同意**，扫描用户指定范围（`~/.workbuddy/skills/` 等）子技能的该文件，按 `id` 去重汇入 `references/factory-log.md`（`status: pending`）；
  - 手动兜底：用户把子技能（含 `references/memory/`）喂回走再进化（§12），或直接粘贴反馈文本。
- **修复边界（低风险自动 / 高风险人工）**：机制类同类 **≥3 条（默认）** 触发工厂自检；低风险（补字段/补示例/改文案/修文档笔误/补工具命令，不触碰抽取矩阵与 Schema）→ 自动应用 + 回归护栏（两路径 `quick_validate.py` + `py_compile` + 旧功能抽查）→ 通过则 `patch++` 并记 `adopted`，失败回滚；高风险（改抽取矩阵/Schema/协议/加领域预设）→ 只自动归因 + 出建议清单，落地需用户明确确认（遵循"工厂技能改写回惯例：用户决定制"）；宿主类不自动修。
- **协议落点**：`evolution-protocol.md` 新增 §14（14.1 分诊 / 14.2 双通道 / 14.3 修复边界 + 回归护栏 / 14.4 与既有机制关系）+ §1 记忆库加 `factory-feedback.json` + §6 元进化触发补"机制类 ≥3 条" + §8 参数表加 3 项 + §13 补衔接段；`SKILL.md` 新增「运行期反馈回传」小节；`distillation-method.md` §18 补闭环。
- 工厂版本 v3.6.1 → v3.7.0；抽取 Schema 仍 v3.6（未改抽取矩阵，仅进化协议与文档增强）。
...
- **verify-protocol**：失败原因分类新增 `合并错误`；§4 回流修正与 §8 回归验证挂载合并降级（降级后重跑受影响 gold）。
- **SKILL.md**：跨次合并四步③同义合并与合并清单同步（待验证项非人工阻塞）；④冲突裁决保留「权威×置信 + 人工确认」。
- 工厂版本 v3.4 → v3.5；Schema 仍 v3.0（未改抽取矩阵，仅合并策略与验证机制）。

## v3.4（2026-08-25）— 竖排文字感知（竖版页面 / 古籍 / 竖排图文 OCR 重组）
- **新增 `scripts/ocr_layout.py`**：竖排文字感知的 OCR 结果重排器（标准库，零依赖）——输入 OCR 文本块 + 边框坐标（JSON，兼容 PaddleOCR/rapidocr 四点框与行式），自动**检测竖排**（窄块宽≤高×0.7 占比≥50%）→ **按列聚类**（x 中心）→ 列内按 y 从上到下 → 列序默认**右起**（中文传统 rtl，`--column-order ltr` 可左起）；`--mode auto|vertical|horizontal` 与 `--mark-columns` 可选。横排输入也统一做「y 分行、行内 x 排序」的阅读顺序整理。
- **协议**：distillation-method 第 17 节新增「竖排文字处理」三步（检测 → 重组 → 旋转 90° 兜底，竖排转横排再 OCR + 坐标反变换）；`source_media.note` 新增 `vertical_text` 标注。
- **工具链**：media-ingest-guide 新增 §4.5 OCR 竖排处理（三引擎输出转统一 JSON 的写法、ocr_layout 用法、ffmpeg/Pillow 旋转兜底命令）。
- **SKILL.md**：第 1 步新增「图片 / 扫描件 / 竖版页面」输入分类（截图/竖版 PDF/古籍/竖版海报/长图 → OCR + 阅读顺序整理 + 竖排重组）；画面 OCR 条目补竖排提示；description 增加竖排/古籍/竖版文档触发词。
- 工厂版本 v3.3 → v3.4；Schema 仍 v3.0（未改抽取矩阵，仅媒体摄入协议与工具增强）。
