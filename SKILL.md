...
### 运行期反馈回传（子技能问题 → 工厂 · v3.7 新增）
子技能在真实使用中遇到"自己解决不了的问题"时，工厂能收到信号并自动归因处理——**不是所有问题都要用户手动转述**：
- **分诊三型**（回传前先归因）：`content` 内容类（改子技能自身规则/知识就能修）→ 子技能走升级环，不回传；`mechanism` 机制类（只有改**工厂产出逻辑**才能根治，如 SKILL.md 结构缺字段/schema 不兼容/脚本报错）→ **回传工厂**；`host` 宿主类（重启/授权/换通道就能解，如 C 类沙箱故障）→ 生成宿主 Bug Report 模板交用户上报，**不当作工厂缺陷**。
- **双通道回传**：①推模式——子技能升级环执行时自动写 `feedback/factory-feedback.json`；②拉模式——工厂每次被调用时（先征得用户同意）扫描用户指定范围子技能的该文件，未读条目汇入 `references/factory-log.md`（`status: pending`）。手动喂回（§12 再进化）仍兜底。
- **修复边界**：机制类同类 ≥3 条触发工厂自检；**低风险**（不触碰抽取矩阵/Schema）→ 自动应用 + 回归护栏（两路径 quick_validate + py_compile + 旧功能抽查）→ 通过则 `patch++`，失败回滚；**高风险**（改矩阵/Schema/协议）→ 只自动归因 + 出建议清单，落地需用户明确确认（遵循"工厂技能改写回惯例：用户决定制"）；宿主类不自动修。
- 完整协议见 references/evolution-protocol.md §14。
